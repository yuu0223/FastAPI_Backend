...
async def edit_member_info(
    editMemberInfoRequest: schema.EditMemberInfoRequest, db: Session = Depends(get_db)
):
    return await business.edit_member_info(editMemberInfoRequest, db)


@router.patch("/forget_pwd", summary="會員忘記密碼")
async def forget_pwd(email: str, db: Session = Depends(get_db)):
    return await business.forget_pwd(email, db)
